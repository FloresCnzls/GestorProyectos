  ...
  const actualizarTarea = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setTareas(
          tareas.map((tarea) => (tarea.id === editTareaId ? tareaActualizada : tarea))
        );
        closeModal();
      }
    } catch (error) {
      ...
      <div className={styles.container}>
        ...
        <div id="editModal" className={`${styles.modal} ${activeModal === 'editModal' ? styles.active : ''}`}>
          <div className={styles.modalContent}>
            ...
            <div className={styles.modalBtnContainer}>
              <button className={styles.modalBtn} onClick={editTareaId ? actualizarTarea : crearTarea}>
                {editTareaId ? 'Guardar Cambios' : 'Crear Tarea'}
              </button>
            </div>
          </div>
        </div>
        ...
        <div id="deleteModal" className={`${styles.modal} ${activeModal === 'deleteModal' ? styles.active : ''}`}>
          <div className={styles.modalContent}>
            ...
            <h3>¿Estás seguro de que quieres eliminar la tarea?</h3>
            <button className={styles.deleteConfirmBtn} onClick={() => eliminarTarea(editTareaId)}>Eliminar</button>
          </div>
        </div>
      </div>
    </div>
